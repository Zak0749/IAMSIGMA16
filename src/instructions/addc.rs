...
impl Instruction<1> for AddC {
    fn execute(&mut self, data: &mut Machine) -> Line {
        let (result, overflow) = data.register(self.r_a)
            + &data
                .register(self.r_b)
                .with_carry(data.register(Register::R15).get_bit(7));
        ...
        *data.register_mut(Register::R15) = Word::from(Flags {
            ...
            c: matches!(overflow, Overflow::UnsignedOverflow),
            ..Default::default()
        });
